...
class LongitudinalAutoPilot(object):
    def __init__(self):
        ...
        self.gain_p_pitch = 5.0
        self.gain_p_q = 1.0
        
        self.gain_p_alt = 0.03
        self.gain_i_alt = 0.01
        
        self.throttle_ff = 0.67
        self.gain_p_speed = 0.5
        self.gain_i_speed = 0.2
        
        self.gain_p_speed2 = -0.2
        self.gain_i_speed2 = -0.2
        
        self.altitude_switch = 25.0
    ...
    def pitch_loop(self, pitch, pitch_rate, pitch_cmd):
        ...
        # START SOLUTION
        gain_p_pitch = self.gain_p_pitch
        gain_p_q = self.gain_p_q
    ...
    def altitude_loop(self, altitude, altitude_cmd, dt):
        ...
        gain_p_alt = self.gain_p_alt
        gain_i_alt = self.gain_i_alt
    ...
    def airspeed_loop(self, airspeed, airspeed_cmd, dt):        
        ...
        # START SOLUTION
        throttle_ff = self.throttle_ff
        gain_p_speed = self.gain_p_speed
        gain_i_speed = self.gain_i_speed
    ...
    def airspeed_pitch_loop(self, airspeed, airspeed_cmd, dt):
        ...
        # START SOLUTION
        gain_p_speed2 = self.gain_p_speed2
        gain_i_speed2 = self.gain_i_speed2
        
        airspeed_error = airspeed_cmd-airspeed
        self.climb_speed_int = self.climb_speed_int + airspeed_error*dt
       
        pitch_cmd_unsat = gain_p_speed2*airspeed_error + gain_i_speed2*self.climb_speed_int
        ...
        # Anti wind-up
        if(gain_i_speed2 != 0):
            self.climb_speed_int = self.climb_speed_int + dt/gain_i_speed2*(pitch_cmd - pitch_cmd_unsat)
    ...
    def longitudinal_loop(self, airspeed, altitude, airspeed_cmd, altitude_cmd,
        ...
        # START SOLUTION
        altitude_switch = self.altitude_switch
...
class LateralAutoPilot:
    ...
    def __init__(self):
        ...
        self.gain_p_roll = 10.0
        self.gain_p_p = 1.0

        self.gain_p_yaw = 3
        self.gain_i_yaw = 0.2

        self.gain_p_sideslip = -2.0
        self.gain_i_sideslip = -1.0
    ...
    def roll_attitude_hold_loop(self,
        ...
        # START SOLUION
        gain_p_roll = self.gain_p_roll
        gain_p_p = self.gain_p_p
        aileron = gain_p_roll*(phi_cmd-phi) - gain_p_p*roll_rate
        # END SOLUTION
        return aileron
    ...
    def yaw_hold_loop(self,
        ...
        # START SOLUTION
        gain_p_yaw = self.gain_p_yaw
        gain_i_yaw = self.gain_i_yaw
    ...
    def sideslip_hold_loop(self,
        ...
        # START SOLUTION
        gain_p_sideslip = self.gain_p_sideslip
        gain_i_sideslip = self.gain_i_sideslip
        
        self.integrator_beta = self.integrator_beta+(0.0-beta)*T_s
        rudder_unsat = gain_p_sideslip*(0.0-beta)
        
        if(np.abs(rudder_unsat) > 1):
            rudder_unsat = 1*np.sign(rudder_unsat)
        
        rudder_unsat = rudder_unsat + gain_i_sideslip*self.integrator_beta
        ...
        if(gain_i_sideslip != 0):
            self.integrator_beta = self.integrator_beta + (T_s/gain_i_sideslip)*(rudder-rudder_unsat)
        #END SOLUTION
        return rudder
    ...
    def straight_line_guidance(self, line_origin, line_course, 
        ...
        # START SOLUTION
        gain_p_xtrack = self.gain_p_xtrack
